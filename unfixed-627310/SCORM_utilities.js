var CONFIG = {
				...
				FILENAME			: "Health Insurance Open Enrollment.swf",
				BGCOLOR				: "#037b94",
				FPVERSION			: "10",
				COURSE_WIDTH		: "710",
				COURSE_HEIGHT		: "540",
...
var strLMSScores;
var strLMSPointScores;
var strLMSChildren;
var strLMSProgress;
var strLMSPassFail;	
function isValidVar(aVar)
{
	if(typeof(aVar) === "undefined" || aVar == null)
		return false;
	return true;	
}
	...
	*/
    ...
    if(!lRunningInConnect)
    {
    	if(CaptivateObj) 
    	{
    		...
    		if(isValidVar(strLMSLocation))
    		{
    			SetBookmark(strLMSLocation);
    		}
    		if(isValidVar(strLMSTime))
    		{
    			SetSessionTime(strLMSTime);
    		}
    		if(isValidVar(strLMSSuspendData))
    		{
    			SetDataChunk(strLMSSuspendData);
    		}
    		if(isValidVar(strLMSScores))
    		{
    			var scores = strLMSScores;
    			SetScore(scores[0], scores[1], scores[2]);
    		}
    		if(isValidVar(strLMSPointScores))
    		{
    			var scores = strLMSPointScores;
    			SetPointBasedScore(scores[0], scores[1], scores[2]);
    		}
    		if(isValidVar(strLMSProgress))
    		{
    			SetProgressMeasure(strLMSProgress);
    		}
    		if(isValidVar(strLMSPassFail))
    		{
    			if(strLMSPassFail == "Pass")
    				SetPassed();
    			else if(strLMSPassFail == "Fail")
    				SetFailed();
    		}
    		if(isValidVar(strLMSStatus) && strLMSStatus == "Set")
    		{
    			SetReachedEnd();
    		}
    		ConcedeControl();
